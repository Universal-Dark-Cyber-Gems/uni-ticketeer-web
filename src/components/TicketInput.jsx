export default function TicketInput({type, required=false, placeholder, tagType, options, value, name, label, onChange, min }){
    return(
        <div className="w-full my-2">
            ...
            <div className="w-full border-2 border-primary-dark rounded-md bg-[#F7F7F7]">
                {tagType === "input" && <input type={type} placeholder={placeholder} name={name} required={required} value={value} onChange={onChange} min={min} className="w-full bg-transparent placeholder:text-primary-dark outline-none p-[2px]" /> }
